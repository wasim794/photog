...
if(isset($_REQUEST['fid'])){
 $fid=$_REQUEST['fid'];
  ...
  $sql1 = "select * from providers where fid='$fid'";
...
$error = false;
	...
	$type=array("image/jpg","image/jpeg","image/PNG","image/png","image/gif","image/bmp"); 
		...
		if( !$error ) {
			...
			$query = "INSERT INTO providers(category,title,status,ufile) 
			VALUES('$category','$title','$status','$ufile')";
	...
	$type=array("image/jpg","image/jpeg","image/PNG","image/png","image/gif","image/bmp"); 
		if( !$error ) {
		 if($_name!="")
           {		   
		    $sql=$conn->query("select * FROM `providers` WHERE fid='$fid'");
        	$row = $sql->fetch_array();
        	$filename = $row['ufile'];
...
<ol class="breadcrumb">
  <li class="breadcrumb-item"><a href="dashboard.php">Dashboard</a><i class="fa fa-angle-right"></i>Add Provider Image </li>
            </ol>
