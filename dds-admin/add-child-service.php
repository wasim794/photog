...
if (isset($_POST['save']) && empty($_POST['id']) ) {
	...
	if(empty($_name2)){$ufile1='';}
		...
		if (empty($child)) {
			$error = true;
			$categoryError = "Please Select child.";
		} 
		...
		if (empty($description)) {
			...
			$query = "INSERT INTO service(child,sub, title, ufile, ufile1, description, works, why, status, rstatus, video) VALUES('$child', '$sub', '$title' , '$ufile', '$ufile1','$description', '$works', '$why', '$status', '$rstatus','$video')";
	 ...
	 if(!empty(array_filter($_FILES['files']['name']))){
	  ...
	  if(!empty(array_filter($_FILES['files1']['name']))){
		  ...
		  foreach($_FILES['files1']['name'] as $key=>$val){
 			...
 			if ($res) {			
		       header('location:view-child-service.php');	
			} else {
				$errTyp = "danger";
...
function chkDelete(id,pid){
	if(confirm("Are you sure you want to delete Record(s)"))	{
		window.location.href="add-child-service.php?id="+id+"&delete="+pid;
	}
	else {return false;}
}
function chkDelete1(id,pid){
	if(confirm("Are you sure you want to delete Record(s)"))	{
		window.location.href="add-child-service.php?id="+id+"&delete1="+pid;
	}
	else {return false;}
}
</script>
...
<div class="col-md-6">
<div class="form-group">
<label for="Industry" class="ilable1"><strong>Child </strong></label>
<select name="category" class="form-control"  id="chate">
	<option value="">Select Any One</option>	
<?php  $sql1 = "select * from child order by cate asc";
				  $query1 = $conn->query($sql1);
				  while($row = $query1->fetch_array()){
				  $cate= $row['cate'];
				   $cid= $row['id'];
				  ?>
			 	 <option value="<?php echo $cid;?>" <?php if($child==$cid){ echo 'selected="selected"';}?>>
